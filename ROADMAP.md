...
### 7. Функции в выражениях — уже работает
Вызовы функций внутри выражений работают и в `do_e_axmath2`, и в `doregmath_32`.
Примеры: `a = foo(b) + 1`, `a = b + foo(b) * 2`, `a = b * foo(b) + 1`.
Интеграция через `procdo()` в term-loop (case `tk_ID`/`tk_proc` и т.д.).

### 8. `doregmath64` — 64-bit math — `tokb.cpp:23459`
